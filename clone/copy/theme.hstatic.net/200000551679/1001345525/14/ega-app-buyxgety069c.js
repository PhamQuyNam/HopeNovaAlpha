...
(function($){
	...
	buyXgetY = {
		...
		UpdateCartFromCart: function() {debugger;
				...
				if(result != null){
					$.each(result,function(i,item){
						...
						if (item.has_gift == true || item.has_discount == true){
							...
							$.getJSON("https://buyxgety-omni.haravan.com/js/recommendeds?product_id=" + item.product_id, function(result2){
								$('.quatang[data-id="'+ item.product_id +'"]').html('<span>Tặng ' + result2.recommendeds[0].product_name.replace('[HÀNG TẶNG KHÔNG BÁN] ', '') + '</span')						
							})
							...
							$('.quatang[data-id="'+ item.product_id +'"]').remove()
